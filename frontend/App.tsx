...
import { processChunkWithLLM, initializeSession, LLMSession, loadPresets } from './services/llmService';
...
function App() {
  ...
  const [currentPromptName, setCurrentPromptName] = useState<string>('humorous_rewrite');
  ...
  useEffect(() => {
    loadPresets().then(data => {
      ...
      setCurrentPromptName(data.current_prompt || 'humorous_rewrite');
  ...
  }, [appConfig, appConfig.systemPrompt, isContextual, sourceText, isGlossaryEnabled, glossaryPrompt, glossaryTerms]);
  ...
  }, [currentPromptName, promptPresets, appConfig.systemPrompt]);
  ...
  const processQueue = useCallback(async (currentChunks: ChunkItem[]) => {
    ...
    if (!isParallel && isContextual && !sessionRef.current) {
      sessionRef.current = initializeSession({ promptName: currentPromptName });
    }
  ...
  }, [isParallel, concurrencyLimit, isContextual, currentPromptName]);
  ...
  useEffect(() => {
    ...
    toStart.forEach(chunk => {
      ...
      processChunkWithLLM(
        ...
        (!isParallel && isContextual) ? sessionRef.current : undefined
      )
        .then(result => {
  ...
  }, [chunks, currentPromptName, isContextual, isParallel, updateChunkStatus]);
  ...
  const effectiveSystemPrompt = getSystemPrompt();

  if (!presetsLoaded) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-brand-light">
        <div className="text-center">
          <div className="w-12 h-12 border-2 border-brand-orange border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-stone-500 font-serif italic">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Sidebar
        ...
        onPromptChange={setCurrentPromptName}
      />
