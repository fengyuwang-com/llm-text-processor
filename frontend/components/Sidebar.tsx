      ...
      <div className={`flex-1 overflow-y-auto custom-scrollbar px-6 py-4 space-y-8 ${disabled ? 'opacity-50 pointer-events-none grayscale-[0.3]' : ''}`}>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            {Object.keys(promptPresets).length > 0 && (
              <div className="animate-fade-in group">
                <label className="flex justify-between text-xs font-medium text-stone-500 mb-2 group-focus-within:text-brand-orange transition-colors">
                  Preset
                </label>
                <div className="space-y-1">
                  {Object.entries(promptPresets).map(([key, preset]) => (
                    <button
                      key={key}
                      onClick={() => onPromptChange?.(key)}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                        currentPromptName === key
                          ? 'bg-brand-orange/10 text-brand-orange border border-brand-orange/20'
                          : 'bg-white text-stone-600 border border-stone-200 hover:border-brand-orange/30 hover:text-brand-orange'
                      }`}
                    >
                      <div className="font-semibold font-sans">{preset.name}</div>
                      {preset.description && (
                        <div className="text-[10px] text-stone-400 mt-0.5 font-serif">{preset.description}</div>
                      )}
                    </button>
                  ))}
                </div>
      ...
      <div className="px-6 py-3 bg-stone-50 border-t border-stone-200 text-center shrink-0">
        <p className="text-xs font-medium text-stone-400 tracking-wide font-sans flex items-center justify-center gap-2">
          <a
            ...
            title="UI by AI-Long-Text-Flow"
          >
            <Github size={18} />
          </a>
          UI: <a href="https://github.com/zlhhhh8901/AI-Long-Text-Flow" target="_blank" rel="noopener noreferrer" className="text-stone-400 hover:text-brand-orange transition-colors underline underline-offset-2">AI-Long-Text-Flow</a> by zlhhhh8901
        </p>
      </div>
    </div>
  );
